...
const STORAGE_KEY = 'techtor_learning_progress_v2';
const CUSTOM_PATHS_KEY = 'techtor_custom_learning_paths_v2';
...
export const LearningPathsView: React.FC<LearningPathsViewProps> = ({
  ...
  // Initial state with defaults for hydration safety
  const [paths, setPaths] = useState<LearningPath[]>(DEFAULT_LEARNING_PATHS);
  const [selectedPathId, setSelectedPathId] = useState<string>('track-ts-architecture');
  const [selectedModuleId, setSelectedModuleId] = useState<string>('ts-arch-mod-1');
  ...
  // Restore persisted learning state after mount
  useEffect(() => {
    try {
      const customSaved = localStorage.getItem(CUSTOM_PATHS_KEY);
      if (customSaved) {
        const parsedCustom = JSON.parse(customSaved);
        if (Array.isArray(parsedCustom) && parsedCustom.length > 0) {
          const customIds = new Set(parsedCustom.map((p: LearningPath) => p.id));
          const filteredDefaults = DEFAULT_LEARNING_PATHS.filter((p) => !customIds.has(p.id));
          setPaths([...parsedCustom, ...filteredDefaults]);
        }
      }

      const progressSaved = localStorage.getItem(STORAGE_KEY);
      if (progressSaved) {
        const parsedProgress = JSON.parse(progressSaved);
        if (parsedProgress.completedModules) setCompletedModules(parsedProgress.completedModules);
        if (parsedProgress.quizAnswers) setQuizAnswers(parsedProgress.quizAnswers);
        if (parsedProgress.capstoneChecklist) setCapstoneChecklist(parsedProgress.capstoneChecklist);
      }
    } catch {
      // ignore
    }
    ...
    <div className="flex-1 flex flex-col h-[calc(100vh-60px)] sm:h-[calc(100vh-65px)] bg-[#0d1117] overflow-hidden text-slate-100 font-sans">
      ...
      <div className="lg:hidden flex items-center justify-between px-3 py-2 bg-[#161b22] border-b border-slate-800 flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1.5 min-h-[36px] rounded-lg bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold transition shadow-sm whitespace-nowrap flex-shrink-0"
        >
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        ...
        >
          ...
          <div className="p-3.5 border-b border-slate-800 space-y-2.5 flex-shrink-0">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setShowGenerateModal(true)}
                className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 min-h-[34px] rounded-lg bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold transition shadow-sm whitespace-nowrap flex-shrink-0"
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-900 border border-slate-800 rounded-lg text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-teal-500"
              />
              {searchQuery && (
                <button
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3.5 rounded-xl transition flex flex-col gap-2.5 ${
                      isSelected
                        ? 'bg-teal-950/70 border border-teal-500/50 text-white shadow-md'
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-2.5 space-y-2 text-xs min-h-0">
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border transition cursor-pointer flex items-start gap-2.5 ${
                    isSelected
                      ? 'bg-slate-800/90 border-teal-500/60 shadow-sm text-white'
                ...
                >
                  ...
                  >
                    {isCompleted ? (
                      ...
                      <div className="w-4 h-4 rounded-full border-2 border-slate-600 hover:border-teal-400 flex-shrink-0" />
                    )}
                  </button>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                      <span className="text-[10px] font-bold text-teal-400 uppercase whitespace-nowrap">
                        Week {mod.week}
                      </span>
                      {mod.language && (
            ...
            <div
              ...
              className="p-3.5 rounded-xl bg-gradient-to-br from-teal-950/50 via-purple-950/30 to-slate-900 border border-teal-500/30 text-xs cursor-pointer hover:border-teal-400 transition space-y-1.5"
            >
              <div className="flex items-center justify-between">
        ...
        >
              ...
              {currentModule.quiz && currentModule.quiz.length > 0 && (
                ...
                >
                  ...
                  {currentQuizScore && currentQuizScore.answered > 0 && (
                    <span className="ml-1 px-1.5 py-0.2 rounded-full text-[10px] bg-slate-800 text-teal-300">
          ...
          {activeLessonTab === 'theory' && (
            <div className="space-y-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-teal-600 hover:bg-teal-500 text-white text-[11px] font-semibold transition"
                      >
              ...
              <div className="pt-2 flex items-center justify-end">
                <button
                  onClick={() => setActiveLessonTab('challenge')}
                  className="flex items-center gap-1.5 px-4 py-2 min-h-[38px] rounded-lg bg-teal-600 hover:bg-teal-500 text-white font-semibold transition shadow-md shadow-teal-600/20 text-xs"
                >
          ...
          {activeLessonTab === 'challenge' && (
            <div className="space-y-5">
              <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-teal-950/40 via-slate-900 to-teal-950/20 border border-teal-500/30 space-y-4 shadow-xl">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="px-2.5 py-1 rounded text-[11px] font-semibold bg-teal-500/20 text-teal-200 border border-teal-500/30 whitespace-nowrap flex-shrink-0">
                ...
                <div className="pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-5 py-2.5 min-h-[40px] rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-bold transition shadow-lg shadow-teal-600/30 whitespace-nowrap flex-shrink-0 text-xs sm:text-sm"
                  >
          ...
          {activeLessonTab === 'quiz' && (
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-[#161b22] border border-slate-800 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {currentQuizScore && (
                    <span className="px-2.5 py-1 rounded-lg bg-teal-950/60 border border-teal-500/40 text-teal-300 font-bold text-xs">
          ...
          {activeLessonTab === 'capstone' && (
            <div className="space-y-5">
              {/* Capstone Card */}
              <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-teal-950/50 via-purple-950/30 to-slate-900 border border-teal-500/40 space-y-4 shadow-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
            ...
            <form onSubmit={handleGenerateCustomPath} className="space-y-3.5 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-teal-500 min-h-[38px]"
                />
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {SUGGESTED_TOPICS.map((topic) => (
                    <button
                      ...
                      className="px-2 py-0.5 rounded-md bg-slate-900 hover:bg-slate-800 border border-slate-800 text-[10px] text-slate-400 hover:text-teal-300 transition"
                    >
                      + {topic}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-200 focus:outline-none focus:border-teal-500 min-h-[38px]"
                  >
              ...
              {isGeneratingPath && (
                <div className="p-3.5 rounded-xl bg-teal-950/40 border border-teal-500/30 space-y-2">
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-5 py-2 min-h-[38px] rounded-lg bg-teal-600 hover:bg-teal-500 text-white font-bold transition shadow-md shadow-teal-600/30 disabled:opacity-50 whitespace-nowrap"
                >
                  {isGeneratingPath ? (
                    <>
          ...
          <div className="w-full max-w-md p-6 rounded-2xl bg-gradient-to-b from-[#161b22] to-slate-900 border border-amber-500/40 shadow-2xl space-y-4 text-center">
            ...
            <div className="pt-2 flex justify-center">
              <button
                ...
                className="px-6 py-2.5 min-h-[38px] rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs transition shadow-lg shadow-teal-600/30"
              >
                Close Certificate
              </button>
            </div>
